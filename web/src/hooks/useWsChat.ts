...
export function useWsChat({ tenantId, sessionId, userId, enabled = true }: Opts) {
  ...
  useEffect(() => {
    ...
    // Don't wipe items here — keep the cached timeline visible during
    // reconnect. The dedup-by-seq guard in onmessage handles overlap when
    // the server replays prior replies via replies_since(0).
    setError(null);
